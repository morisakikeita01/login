<template>
    <div class="chara-container">
        ...
        <swiper
            ...
            class="mySwiper" >
            <swiper-slide v-for="(char, idx) in props.characters" :key="idx" >
                <p class="char" 
                    :style="{backgroundImage: 'url(/images/charas/' + char + '.png)', backgroundSize: 'cover'}"
                    :class="{active:props.modelValue['character'].value===char, disabled:!['soldier', 'robot'].includes(char)}" 
                    @click="selectChara(char)">
                    <!-- <img :src="'/images/charas/' + char + '.png'"> -->
                </p>
            </swiper-slide>
            <div class="nav flex justify-center">
                <i class="fas fa-chevron-left" :class="{disabled:slideActiveIndex===0}" @click="slideToPrev"></i>
                <span>　　</span>
                <i class="fas fa-chevron-right" :class="{disabled:slideActiveIndex===props.characters.length-slidesPerView}" @click="slideToNext"></i>
            </div>
        </swiper>
    </div>
    <!-- error message -->
    <p v-if="props.modelValue['character'].error" class="errors">
        <span v-for="(error, idx) in  props.modelValue['character'].error" :key="idx">{{error}}</span>
    </p>
</template>
